...
// Create an ArrayAdapter using the string array and a default spinner layout.
    ...
    fun onClickOceny(v: View) {
        ...
        zapytanie.put("query","SELECT * FROM dziennik_ocen WHERE przedmiot = '$przedmiot' AND id_ucznia = (SELECT id FROM users WHERE username = \"${MainActivity.username}\");")
    ...
    fun processResponse(response: JSONObject) {
        ...
        for(i in 0 until jsonLista.length()){
            //lista.add("${MainActivity.username}"+" " + findViewById<Spinner>(R.id.spinner).getSelectedItem().toString())
