...
class WiadomosciUczen : AppCompatActivity() {
    ...
    fun onClickOdswiez(v: View){
        ...
        zapytanie.put(
            "query",
            "SELECT * FROM wiadomosci WHERE klasa = (SELECT klasa FROM uczen_klasa WHERE id_ucznia = (select id from users where username  = \"${MainActivity.username}\"));"
        )
